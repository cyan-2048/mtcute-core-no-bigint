...
const DH_SAFETY_RANGE = JSBI.exponentiate(
	JSBI.BigInt(2),
	JSBI.subtract(JSBI.BigInt(2048), JSBI.BigInt(64))
);
...
function checkDhPrime(crypto: ICryptoProvider, log: Logger, dhPrime: JSBI, g: number) {
	...
	if (!checkedPrime) {
		if (
			JSBI.lessThanOrEqual(dhPrime, TWO_POW_2047) ||
			JSBI.greaterThanOrEqual(dhPrime, TWO_POW_2048)
		) {
...
export async function doAuthorization(
	...
	if (
		!buffersEqual(innerDataHash, crypto.sha1(plainTextAnswer.subarray(20, serverDhInnerReader.pos)))
	) {
	...
	for (;;) {
		...
		const clientDhInnerHash = crypto.sha1(
			clientDhInnerWriter.uint8View.subarray(20, clientDhInnerWriter.pos)
		);
		...
		if (dhGen._ === "mt_dh_gen_retry") {
			const expectedHash = crypto.sha1(
				concatBuffers([newNonce, new Uint8Array([2]), authKeyAuxHash])
			);
		...
		const expectedHash = crypto.sha1(
			concatBuffers([newNonce, new Uint8Array([1]), authKeyAuxHash])
		);
