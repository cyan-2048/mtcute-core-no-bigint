...
export function bigIntToBuffer(value: JSBI, length = 0, le = false): Uint8Array {
	...
	for (let i = 0; i < dv.byteLength; i += 8) {
		JSBI.DataViewSetBigUint64(
			dv,
			i,
			JSBI.bitwiseAnd(value, JSBI.BigInt("0xffffffffffffffff")),
			true
		);
...
export function bufferToBigInt(buffer: Uint8Array, le = false): JSBI {
	...
	for (let i = 0; i < dv.byteLength; i += 8) {
		res = JSBI.bitwiseOr(
			JSBI.leftShift(res, JSBI.BigInt(64)),
			JSBI.DataViewGetBigUint64(dv, i, false)
		);
	}
...
export function randomBigIntInRange(
	...
	while (result > interval) result = JSBI.subtract(result, interval);
